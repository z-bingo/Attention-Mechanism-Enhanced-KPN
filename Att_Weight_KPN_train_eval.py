...
def train(config, num_workers, num_threads, cuda, restart_train, mGPU):
    ...
    if not restart_train:
        try:
            ...
            print('load global_iter ok')
            best_loss = checkpoint['best_loss']
            print('load best_loss ok')
            model.load_state_dict(checkpoint['state_dict'])
            print('model load ok')
            optimizer.load_state_dict(checkpoint['optimizer'])
            print('optimizer load ok')
    ...
    with torch.no_grad():
        ...
        for i, (burst_noise, gt, white_level) in enumerate(data_loader):
            if i < 100:
                ...
                trans(burst_noise[0, 0, ...].squeeze()).save(
                    os.path.join(eval_dir, '{}_noisy_{:.2f}dB.png'.format(i, psnr_noisy)), quality=100)
                trans(pred.squeeze()).save(os.path.join(eval_dir, '{}_pred_{:.2f}dB.png'.format(i, psnr_t)),
                                           quality=100)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file', dest='config_file', default='kpn_specs/att_weight_kpn_config.conf', help='path to config file')
    ...
    parser.add_argument('--eval', action='store_true', help='whether to work on the eval mode')
