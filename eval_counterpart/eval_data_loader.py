...
class TrainDataSet(torch.utils.data.Dataset):
    def __init__(self, dataset_dir, burst_size=8, patch_size=192, upscale=4, img_format='.bmp',
                 sigma_read=None, sigma_shot=None, degamma=True, color=True, blind=False):
    ...
    def __getitem__(self, index):
        ...
        if not self.color:
            # 生成随机的read和shot噪声方差
            if not self.sigma_read or not self.sigma_shot:
